...
The adapter searches for all installed BackItUp instances and filters those that meet the prerequisites (CIFS enabled with connection type "Copy").

- **One matching instance:** The Drive Sync tab shows a success message indicating which instance is used (e.g. "Using BackItUp instance: backitup.0"). The backup path is read automatically.
- **Multiple matching instances:** A dropdown appears in the Drive Sync tab to select which BackItUp instance to use. Each entry shows the instance name and backup path.
- **No matching instance:** A warning is displayed that no BackItUp instance with a suitable CIFS configuration was found.

**Adding a BackItUp sync entry:**

1. If multiple BackItUp instances are found, **select the desired instance** from the dropdown first.
2. In the Drive Sync tab, click **"Add BackItUp Sync"** (only visible when at least one instance meets the prerequisites).
3. The **Local Path** is automatically filled from the selected BackItUp instance — you cannot change it.
4. Click **Browse** next to "iCloud Drive Folder" to open the folder browser.
...
5. Optionally set **backup limits**:
...
6. Choose a **conflict resolution** strategy and click **Save**.
